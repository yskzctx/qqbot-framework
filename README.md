...
- **[NapCat (NapCatQQ)](https://github.com/NapNeko/NapCatQQ)** —— 本框架的 QQ 消息
  收发能力完全基于 NapCat 开源项目实现（注入式 OneBot 11 协议端）。NapCat 的全部
  版权归 **NapNeko 团队** 所有，本项目仅作为其上层调用方，按其开源许可进行集成。
  如果本框架对你有用，也请去给 NapCat 点个 Star。
- [OneBot 11](https://github.com/botuniverse/onebot-11) —— 机器人接口标准规范。

### 免责声明

1. 本项目（含 NapCat 依赖）仅供 **个人学习、技术研究**，以及自动化操作
   **自己拥有或经授权使用的 QQ 账号**。将项目部署于自己控制的设备与账号，是使用者的自主决定。
2. **严禁**将本项目用于任何违法违规用途，包括但不限于：垃圾信息群发、骚扰、
   诈骗、引流营销、批量注册、破坏他人账号安全等。由此产生的一切法律后果由使用者自行承担。
3. 使用注入类第三方工具存在 **账号被腾讯风控/限制/封禁的风险**。本框架不修改、
   不存储你的 QQ 密码，但无法消除此类固有风险，请自行评估并以小号操作。
4. 本项目与腾讯官方、NapCat 官方均无隶属关系；项目的功能取决于 NapCat 的能力边界，
   随 QQ 版本更新可能失效，作者不承诺任何可用性保证。
5. 本项目按"现状"开源提供（不含任何明示或默示担保）。任何人下载、复制或使用本项目，
   即视为已阅读并同意本声明全部内容。
6. 如本项目的任何内容侵犯了你的合法权益，请通过 Issue 联系，将第一时间处理。
