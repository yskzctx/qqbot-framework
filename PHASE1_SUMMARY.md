...
## 四点十三、消息数据库功能（2026-09-06）

- 框架自动把收到的每条消息存入 SQLite（QQBotData/data/messages.db）
- 面板新增「数据库」页：统计（总数/大小/每天条数）、消息查询（类型/对象/关键词
  过滤 + 分页）、自动清理规则（保留最近 N 天 / 每天 HH:MM 清空）、手动清空
- 模块接口：bot.db_query / bot.db_stats / bot.db_add（指南第 18 节有示例）
- 清理循环：每小时执行保留期清理，每天到达设定时刻清空全部
- NapCat 注入诊断增强：启动输出写入 QQBotData/NapCat/launch.log，
  注入失败时面板显示 NapCat 原始报错（云电脑注入失败的确切原因
  需用户重试后从日志确认）
