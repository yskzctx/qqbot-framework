...
### 4. MCP 远程控制服务 → 已移除
- 曾实现 Streamable HTTP MCP 服务（10 个远控工具），实测可用；
  按需求于阶段末整体移除，代码与依赖不再随框架分发。

### 5. 隧道助手（TunnelHelper.exe + cloudflared.exe）
- 黑窗口控制台：输入端口（默认 2280 即面板端口，记忆）→ 协议（默认 http2 稳定）→
  显示 trycloudflare.com 公网地址，窗口开着隧道一直有效，断线自动重连
- cloudflared 内核缺失自动下载（ghfast.top 等多镜像，带大小校验）
- 用途：云电脑无公网 IP 时，把管理面板穿透到公网远程访问

> 注：一阶段曾内置 MCP 远程控制服务（端口 2281），后按需求在阶段末整体移除
> （代码、配置、依赖均已删除；排查结论保留在"已知问题"里，供隧道使用参考）。
...
## 四点七、云电脑远程升级（2026-09-06）

- 通过云电脑 MCP（16 工具）远程完成升级：下载新 EXE（SHA256 校验一致）→
  旧版备份为 QQBot_old.exe → 替换 → 安装官方 8 个模块 → 启动并验证面板响应
- 云电脑 QQBotData 原有配置/数据完整保留（面板 need_setup=false 验证通过）
- 注意：permissions.admins 需用户在云电脑 config.json 中填入自己的 QQ 号
  （系统工具模块依赖它）

## 四点八、云电脑管理员配置 + BOM 兼容修复（2026-09-06）

- 管理员 QQ 1710049463 已配置到云电脑 config.json
- 排查并修复：PowerShell 5.1 `Out-File -Encoding utf8` 写配置带 BOM，
  导致 json.load 解析失败、框架静默退出；核心已改用 utf-8-sig 读取（免疫 BOM），
  云电脑 EXE 已同步升级并验证（哈希一致、面板正常、管理员生效）
- 云电脑现有 8 个模块 + 全新版核心 + 管理员配置，远控升级流程已跑通

## 五、第二阶段候选事项

- 云电脑上跑通隧道 → 浏览器远程访问管理面板（排障结论见"已知问题"）
